...
--- Apply a new binary for a server and restart it.
---@param server string
---@param binary string
local function apply_binary(server, binary)
  cmd_tables[server][1] = binary
  require("lspconfig")[server].setup({ cmd = { binary } })
    ...
    if choice == "Custom path..." then
      vim.ui.input({ prompt = server .. " executable path: " }, function(input)
        if input and input ~= "" then
          apply_binary(server, input)
        end
      end)
    else
      apply_binary(server, choice)
    end
  end)
end
